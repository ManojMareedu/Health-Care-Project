...
> **Transforming raw Medicare claims data into actionable cost-risk intelligence — combining multi-source data integration, frequency-encoded clinical features, and ensemble classification to predict patient total charge tiers, enabling precision resource allocation and proactive cost containment strategies.**
...
| **Total Charge Range** | Inpatient CLM_PMT_AMT ranging from $62.44 to $598,716.31; outpatient from $59.64 to $428,631.86 |
...
| **Cost Tier Design** | Five classes defined: Class 1 (<$1K), Class 2 ($1K–$10K), Class 3 ($10K–$100K), Class 4 ($100K–$1M), Class 5 (>$1M) |
| **Data Engineering** | Inpatient and outpatient records filtered to shared beneficiaries; claims aggregated by beneficiary-diagnosis-state groupings before merge |
...
|---|---|---|
| `inpatient.csv` | Medicare inpatient claims with 197 columns including ICD diagnosis codes, provider identifiers, DRG codes, claim payment amounts, and admission indicators | 58,066 rows |
| `outpatient.csv` | Medicare outpatient claims with 162 columns including revenue center codes, charge amounts, rendering physician NPIs | 575,092 rows |
...
- `CLM_E_POA_IND_SW1` — Present-on-Admission indicator (dummy-encoded)
- `Number_of_Claims_inp` / `Number_of_Claims_out` — Claim volume per beneficiary-diagnosis grouping
...
- Loaded raw Medicare inpatient (58,066 rows × 197 cols) and outpatient (575,092 rows × 162 cols) claim files
- Standardized beneficiary and claim IDs; filtered outpatient records to beneficiaries with matching inpatient claims
- Selected 7 key features per dataset; labeled patient type (inpatient=1, outpatient=0)
- Computed `Number_of_Claims` via grouped count transform (beneficiary × diagnosis × state)
...
- Computed `TOTAL_CHARGE` as sum of inpatient and outpatient charges per patient record
- Merged with `Median_Income.xlsx` on provider state code to incorporate socioeconomic context
- Applied **frequency encoding** to high-cardinality ICD-10 diagnosis codes (replacing categorical codes with their frequency counts in the dataset)
...
- **K-Nearest Neighbors (KNN):** Grid search over k ∈ {3, 5, 7, 9, 11} with 5-fold cross-validation
- **Decision Tree:** `rpart` with `minbucket=10`, `cp=0`; pruned via complexity parameter plot
- **Random Forest:** 100 trees with variable importance evaluation; compared against KNN and Decision Tree via confusion matrix and multiclass AUC (pROC)
...
```
Raw Medicare Claims (inpatient.csv + outpatient.csv)
            │
            ▼
  [Python — Healthcareprojectfinal.ipynb]
  ├── Data Loading & Type Casting
  ├── Feature Selection (7 cols each)
  ├── Beneficiary Intersection Filter
  ├── Claims Aggregation (groupby)
  ├── Inpatient–Outpatient Merge (on BENE_ID)
  └── Export → Patient_Claim_Data.xlsx
            │
            ▼
  [R — HealthCare Class Project.R]
  ├── Load Patient_Claim_Data.xlsx + Median_Income.xlsx
  ├── TOTAL_CHARGE Engineering
  ├── State-Income Merge
  ├── Frequency Encoding (ICD-10 Dx codes)
  ├── Dummy Encoding (POA Indicator)
  ├── Log Transform + 5-Class Target Creation
  ├── 70/30 Train-Test Split
  └── Model Training:
      ├── Linear Regression (log TOTAL_CHARGE)
      ├── KNN (grid search, 5-fold CV)
      ├── Decision Tree (rpart)
      └── Random Forest (100 trees, varImp)
```
...
**Variable Importance (Random Forest):** Diagnosis frequency encoding for both inpatient and outpatient ICD codes, claim volume (inpatient and outpatient), and median income were the top predictors of charge tier classification.

*Refer to `rf variable importance.png`, `Total charge distribution.png`, and `Lo total charge distribution.png` in the repository for visual outputs.*
...
├── Total charge distribution.png     # Raw total charge distribution
├── Lo total charge distribution.png  # Log-transformed total charge distribution
└── README.md
```
...
1. **Run the Python notebook first** (`Healthcareprojectfinal.ipynb`) with `inpatient.csv` and `outpatient.csv` available in your working directory — this generates `Patient_Claim_Data.xlsx`
2. **Run the R script** (`HealthCare Class Project.R`) with `Patient_Claim_Data.xlsx` and `Median_Income.xlsx` in your working directory — this executes feature engineering and all model training/evaluation
